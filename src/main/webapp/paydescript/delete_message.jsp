    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'><span style='color: red; font-weight: bold;'>해당 월: ${param.month} / 사원명: ${param.employee_name} / 소속: ${param.department_name}</span><br><br> 급여명세서를 삭제했습니다.</LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>[${param.month}월] "${param.department_name} ${param.employee_name}"<br><br>급여명세서 삭제에 실패했습니다.</LI>
        </c:otherwise>
      </c:choose>
      <LI class='li_none'>
        <br>
